...
fn create_deferred_segment(
    ...
    // Vector size: `n_vectors * dim` bytes per vector
    let vector_size = n_vectors * dim * size_of::<f32>();
    let total_vectors = n_vectors + n_deferred;

    let deferred_threshold_bytes = std::num::NonZeroUsize::new(vector_size).unwrap();
